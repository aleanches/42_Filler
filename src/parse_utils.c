...
int	set_player(t_fl **fl, char **line)
{
	...
	if ((tmp = ft_strsplit(*line, ' ')) == NULL || ft_array_len(tmp) < 3)
		put_error(fl, ft_array_free(&tmp, ft_str_free(line, 1)));
...
int	set_map_size(t_fl **fl, char **line)
{
	...
	(*fl)->count_h = 0;
	if ((*fl)->map != NULL)
		return (mtx_clean((*fl)->map, (*fl)->map_w, (*fl)->map_h, 30));
	...
	if (((*fl)->map = mtx_new((*fl)->map_w, (*fl)->map_h)) == NULL)
		put_error(fl, ft_array_free(&tmp, ft_str_free(line, 1)));
...
int	set_token_size(t_fl **fl, char **line)
{
	...
	if (((*fl)->token = mtx_new((*fl)->token_w, (*fl)->token_h)) == NULL)
		put_error(fl, ft_array_free(&tmp, ft_str_free(line, 1)));
...
int	parse_map(t_fl **fl, char **line)
{
	...
	if ((start = ft_get_char_i(*line, ' ')) == -1)
		put_error(fl, ft_str_free(line, 1));
	start++;
